...
queue_on_teleport([[
    loadstring(game:HttpGet("https://raw.githubusercontent.com/yurithome123/5MONEYHUB/main/5money_gpo.lua"))()
]])
...
local CFAHub = 
loadstring(game:HttpGet("https://raw.githubusercontent.com/yurithome123/5MONEYHUB/main/hub.lua?token=GHSAT0AAAAAABXMBXSYFLJBKY25MTXDS7O6YXOTIQA"))()
...
if (executor == "Synapse X" or executor == "Krnl" or executor == "Script Ware") and secure_call then
else
    CFAHub:AddNoti("Warning", "Seu Exploit não é suportado, nossos scripts podem não funcionar corretamente (Supported Exploit: KRNL,Synapse X, Script Ware)", 1)
end
local lf = Section2:CreateToggle("Level Farm", {Toggled=Settings.Farm,Description = "Vai farmar de acordo com seu LVL"}, function(state)
    Settings.Farm = state
    SetEN("Noclip", "Farm", state)
    SetEN("NoFallDame","Farm",state)
end)
 local lf = Section2:CreateToggle("Legit Farm", {Toggled=Settings.Legit,Description = "Legit Farm (TESTE)"}, function(state)
     Settings.Legit = state
 end)

local olf = Section2:CreateToggle("1 Click Level Farm", {Toggled=Settings.OneClick,Description = "Will auto farm beli -> Compre armas e comece o level farm"}, function(state)
    SetEN("Noclip", "OneClick", state)
    SetEN("NoFallDame","OneClick",state)
    Settings.OneClick = state
end)
Section2:CreateToggle("Auto Buso Quest", {Toggled=Settings.AutoBusoQuest,Description = "Nescessario abilitar o Level Farm"}, function(state)
    Settings.AutoBusoQuest=state
end)
Section2:CreateDropdown("Level Farm Metodo", {
...
local AFB= Section2:CreateToggle("Auto Farm Beli (PARA INICIANTES SOMENTE!)", {Toggled=Settings.Chest,Description = "Auto Farm Chest USANDO A SARAH"}, function(state)
...
Section2:CreateTextbox("PS Code", "(Saia se quiser voltar ao servidor normal)", function(args)
    Settings.PSCode=args
end,Settings.PSCode)
local Section2 = Tab2:CreateSection("Local Player")
Section2:CreateToggle("No Drown", {Toggled=Settings.Nodrown,Description = "Faça você não pode se afogar na água"}, function(state)
    Settings.Nodrown = state
    SetEN("Nodrown", "Setting", state)
end)
Section2:CreateToggle("No Fall Damage", {Toggled=Settings.NoFallDame,Description = "Faça você não sofrer nenhum dano quando cair"}, function(state)
    Settings.NoFallDame = state
    SetEN("NoFallDame", "Setting", state)
end)
Section2:CreateToggle("Dash No Stamina", {Toggled=Settings.DashNoStam,Description = "Dash não gasta stamina"}, function(state)
    Settings.DashNoStam = state
    SetEN("DashNoStam", "Setting", state)
end)
 Section2:CreateToggle("No Clip", {Description = "TESTANDO!!- YURI AVISOU SE TOMA BAN!"}, function(state)
...
Section2:CreateTextbox("Web Hook Url", "COLOQUE AQUI!", function(args)
